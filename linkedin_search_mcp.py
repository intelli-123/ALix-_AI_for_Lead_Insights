...
# def _linkedin_urls(query: str, k: int = 5):
#     """Search LinkedIn profiles using Google CSE."""
#     try:
#         svc = build("customsearch", "v1", developerKey=API_KEY)
#         res = svc.cse().list(q=f'site:linkedin.com/in "{query}"', cx=CX_ID, num=k).execute()
#         print("[DEBUG] Google CSE raw response:", res)
#         for item in res.get("items", []):
#             yield item
#     except Exception as e:
...
#     """Return LinkedIn profile URLs and public phone numbers (if any)."""
#     hits = []
#     for item in _linkedin_urls(person_query):
#         url = item.get("link")
#         title = item.get("title", "")
#         snippet = item.get("snippet", "")
#         try:
...
#             phones = list(_phones(txt))
#             print(f"[DEBUG] Extracted phones: {phones}")
...
#                 "designation": title,
#                 "company": snippet,
#                 "location": snippet  # Store full snippet in location as fallback
#             })
#         except Exception as e:
#             print(f"[WARN] Failed to process {url}: {e}")
#             continue

#     return {"query": person_query, "hits": hits}
